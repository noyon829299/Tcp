...
def load_credentials_from_file(filename="HEXOZENTA.txt"):
    """
    Load UID and password from HEXOZENTA.txt file
    """
    try:
        if not os.path.exists(filename):
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]HEXOZENTA GAMING [00FF00]___ HEXOZENTA GAMING____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @hexozenta\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde HEXOZENTA GAMER ya meku agar tu noob bolra toh tu g a y hai
...
║ ────────────── 
║                                       
║ ⚡ [FF00FF] HEXOZENTA GAMER VIP BOT ⚡   
╚═════════════╝"""
                            await safe_send_message(response.Data.chat_type, admin_message, uid, chat_id, key, iv)
...
[00FFFA]║
[FFFF00]║ 👤 Developer    :: [FF1493]HEXOZENTA
